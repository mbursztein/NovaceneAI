...
class Saved_Templates {
    public function __construct(){
        ...
        add_action('init', array($this, 'templates_post_type_callback'));
    ...
    public function custom_head_templates() {
        if( 'ultp_templates' == get_current_screen()->post_type && (!defined('ULTP_PRO_VER')) ) {
            ...
            if( $post_count > 0 ) { ?>
                <span class="ultp-saved-templates-action" data-link="https://www.wpxpo.com/gutenberg-post-blocks/?utm_campaign=go_premium" data-text="Go Pro for Unlimited Templates" data-count="<?php echo $post_count; ?>" style="display:none;"></span>
            <?php }
        }
    }
    ...
    public function disable_new_post_templates() {
        if ( get_current_screen()->post_type == 'ultp_templates' && (!defined('ULTP_PRO_VER')) ){
            ...
            $post_count = $post_count->publish + $post_count->draft;
            if ($post_count > 0) {
                wp_die( 'You are not allowed to do that! Please <a target="_blank" href="https://www.wpxpo.com/gutenberg-post-blocks/?utm_campaign=go_premium">Upgrade Pro.</a>' );
            }
        }        
    }
